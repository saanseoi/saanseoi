...
CREATE TABLE `statsSeriesDimensions` (
	...
	`createdAt` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
	`updatedAt` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
	CONSTRAINT `statsSeriesDimensions_pk` PRIMARY KEY(`seriesId`, `dimensionCode`, `valueCode`)
);
--> statement-breakpoint
...
CREATE INDEX `statsSeriesDimensions_series_idx` ON `statsSeriesDimensions` (`seriesId`);--> statement-breakpoint
...
ALTER TABLE `statsObservations` DROP COLUMN `sourceReleaseId`;--> statement-breakpoint
