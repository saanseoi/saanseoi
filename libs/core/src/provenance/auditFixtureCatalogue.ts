...
/** Row order matches the precomputed catalogue, including deduplicated aliases. */
export function filterIndexedFixture(
  value: Json,
  group: FixtureSearchGroup,
  query: string,
): Json {
  if (!query || !value || typeof value !== 'object' || Array.isArray(value))
    return value
  let offset = 0
  const result = { ...value }
  for (const key of rowKeys) {
    ...
    if (Array.isArray(rows))
      result[key] = rows.filter(() =>
        matchesAuditText(query, group.rows[offset++] ?? ''),
      )
  }
  return result
}
