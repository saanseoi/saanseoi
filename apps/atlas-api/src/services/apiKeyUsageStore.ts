...
export const readUsageCheckpoint = (db: D1Database) =>
  db
    ...
    FROM api_key_usage_rollup WHERE id = 'public-api-usage'`)
    .first<UsageCheckpoint>()

const ownsRevision = `(SELECT revision FROM api_key_usage_rollup WHERE id = 'public-api-usage') = ?`
...
export async function commitUsageWindow(args: {
  ...
  const statements = [
    db
      .prepare(`INSERT INTO api_key_usage_rollup (id, datasets, revision, completed_through)
      VALUES ('public-api-usage', ?, ?, ?)
      ON CONFLICT(id) DO UPDATE SET revision = excluded.revision
      WHERE api_key_usage_rollup.revision = ? AND api_key_usage_rollup.datasets = excluded.datasets`)
    ...
    db
      .prepare(`UPDATE api_key_usage SET request_count = 0
  ...
  for (let offset = 0; offset < rows.length; offset += 250) {
    statements.push(
      db
        .prepare(`INSERT INTO api_key_usage (api_key_id, window, window_started_at, request_count)
      SELECT api_key.id, 'minute', json_extract(value, '$.windowStartedAt'), json_extract(value, '$.requestCount')
      FROM json_each(?) JOIN api_key ON api_key.id = json_extract(value, '$.apiKeyId')
      WHERE ${ownsRevision}
  ...
  for (const period of periods.values()) {
    statements.push(
      ...
      FROM api_key LEFT JOIN api_key_usage AS minute
        ON minute.api_key_id = api_key.id AND minute.window = 'minute'
        AND minute.window_started_at >= ? AND minute.window_started_at < ?
      WHERE ${ownsRevision} AND api_key.id IN (
        SELECT api_key_id FROM api_key_usage WHERE window = 'minute'
  ...
  statements.push(
    db
      .prepare(`UPDATE api_key_usage_rollup SET completed_through = ?
