...
import { getAuditDecision } from '#lib/registry/audit.remote.js'
...
async function load() {
  try {
    result = await getAuditDecision({ releaseId, hash, actionId })
    failure = ''
  } catch {
...
<details ontoggle={event => { if (event.currentTarget.open && !result) void load() }}>
  ...
  <div class="space-y-4 pt-3">
    {#if result}
      <AuditFixture value={result.declaration} />
      {#if result.fixture !== null}
        <AuditFixture value={result.fixture} />
      {/if}
    {:else}
