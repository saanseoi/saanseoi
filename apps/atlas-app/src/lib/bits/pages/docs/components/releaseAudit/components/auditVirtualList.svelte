...
let { items, children }: { items: Json[]; children: Snippet<[Json]> } = $props()
...
<div
  bind:this={viewport}
  class="h-[640px] max-h-[70svh] overflow-auto"
  data-audit-virtual-list
>
  <ul
  ...
  >
    {#each $virtualizer.getVirtualItems() as row (row.key)}
      ...
      >
        {@render children(items[row.index])}
      </li>
    {/each}
  </ul>
</div>
