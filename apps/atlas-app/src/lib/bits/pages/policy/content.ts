...
export const fairUsePolicy: PolicyDocument = {
  version: '1',
  effectiveDate: '11 July 2026',
  contactEmail: 'fairuse@saanseoi.hk',
  i18n: {
    en: {
      title: 'API Fair Use Policy',
      intro: [
        'Saanseoi APIs are shared civic infrastructure. This policy sets baseline limits that preserve free access for small teams, researchers, and public-interest projects while keeping the service dependable for everyone.',
      ],
      sections: [
        {
        ...
        {
          heading: '2. Standard limits',
          paragraphs: [
            'Unless we agree otherwise in writing, each API key has soft limits of 120 requests per minute, 100,000 requests per day, and 1,000,000 requests per calendar month. These limits apply across all API endpoints and reset at the stated interval.',
            'We use these limits as a guide for responsible shared use, not as automatic hard caps. We notify administrators when a key first exceeds a limit in a given time window. A key is temporarily rate-limited only after it exceeds a limit by more than 25%, unless traffic threatens availability.',
          ],
        },
        {
          heading: '3. Efficient use',
          bullets: [
            'Cache responses and respect HTTP cache headers where provided.',
            'Request only the fields, pages, and updates your application needs.',
            'Use conditional requests or incremental updates instead of repeatedly downloading unchanged data.',
            'Avoid concurrent bursts, polling more often than necessary, and bulk re-fetches after transient failures.',
          ],
        },
        {
          heading: '4. Enforcement and responses',
          paragraphs: [
            'Where traffic threatens availability, bypasses safeguards, or aggressively or needlessly exceeds the soft limits, we may introduce a hard limit, throttle traffic, suspend a key, or revoke a key. We will normally contact the account owner first when practical. Daily and monthly limits use UTC calendar windows.',
            'Rate-limited requests return HTTP 429 with a machine-readable error code and a Retry-After value. Invalid, missing, revoked, or suspended keys return a clear authentication error without revealing account information.',
          ],
        },
        {
          heading: '5. Higher-volume and research use',
          paragraphs: [
            'If your public-interest research, service, or integration needs more capacity, email fairuse@saanseoi.hk before running it. Include the API endpoints, expected request rate and monthly volume, caching plan, and a contact address. We will consider reasonable requests case by case.',
          ],
        },
        {
          heading: '6. Policy changes',
          paragraphs: [
            'We may adjust these limits as usage, available capacity, or service design changes. Material changes will be published here and, where practical, communicated to registered API users in advance.',
          ],
        },
      ],
    },
    'zh-hant': {
      title: 'API 公平使用政策',
      intro: [
        '山水 API 是共享的公民基礎設施。本政策訂定基本限額，讓小型團隊、研究人員及公益項目可繼續免費使用，同時確保所有使用者都能可靠地存取服務。',
      ],
      sections: [
        {
          heading: '一、必須使用 API 金鑰',
          paragraphs: [
            '每個 API 請求必須在 x-api-key 標頭中包含有效 API 金鑰。請為不同應用程式建立不同金鑰，並妥善保密。請勿把金鑰嵌入公開網站、流動應用程式、儲存庫或用戶端程式碼。',
          ],
        },
        {
          heading: '二、標準限額',
          paragraphs: [
            '除非我們另有書面同意，每個 API 金鑰的軟性限額為每分鐘 120 個請求、每天 100,000 個請求、每個曆月 1,000,000 個請求。限額適用於所有 API 端點，並按所述時段重設。',
            '我們以此限額作為負責任地共享使用的指引，而非自動執行的硬性上限。當金鑰首次在某個時段超出限額時，我們會通知管理員；只有超出限額逾 25% 時，該金鑰才會暫時被限速，除非流量危及服務可用性。',
          ],
        },
        {
          heading: '三、有效率地使用',
          bullets: [
            '快取回應，並在提供時遵守 HTTP 快取標頭。',
            '只索取應用程式所需的欄位、頁面及更新。',
            '使用條件式請求或增量更新，避免重複下載沒有變更的資料。',
            '避免併發突發流量、不必要的頻繁輪詢，以及暫時性錯誤後的大量重新抓取。',
          ],
        },
        {
          heading: '四、執行及回應',
          paragraphs: [
            '如流量危及服務可用性、規避保障措施，或積極或不必要地超出軟性限額，我們可引入硬性限額、限速、暫停或撤銷金鑰。在可行情況下，我們通常會先聯絡帳戶擁有人。每日及每月限額按 UTC 曆法時段計算。',
            '被限速的請求會回傳 HTTP 429、機器可讀取的錯誤代碼及 Retry-After 值。無效、缺失、已撤銷或已暫停的金鑰會回傳清晰的驗證錯誤，但不會披露帳戶資料。',
          ],
        },
        {
          heading: '五、較高用量及研究用途',
          paragraphs: [
            '如你的公益研究、服務或整合需要更多容量，請在執行前電郵 fairuse@saanseoi.hk。請說明 API 端點、預期請求速率及每月用量、快取方案和聯絡方式；我們會按個案考慮合理申請。',
          ],
        },
        {
          heading: '六、政策更新',
          paragraphs: [
            '我們可能因使用情況、可用容量或服務設計改變而調整限額。重大變更會在此公布，並在可行情況下預先通知已登記 API 使用者。',
          ],
        },
      ],
    },
