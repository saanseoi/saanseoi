  ...
  >
    ...
    <section class="p-4">
      ...
      <p class="mt-3 text-xs leading-5 text-white/55">
        Each district carries its curated code and Area hierarchy item.
      </p>
      <pre
        class="mt-3 max-h-80 overflow-auto border border-white/10 bg-black/15 p-3 text-[11px] leading-5 text-white/70"
      >{JSON.stringify(urbanDensityDivisionsResponse, null, 2)}</pre>
    </section>
  </div>
</div>
