...
export const urbanDensitySetupZ14TileFetcherCode = [
  ...
  '  )',
  '',
  '// Vector-tile fragments were snapped separately, so their finished edges may not line up.',
  '// JSTS is a JavaScript geometry library that makes the finished shapes safe to draw and save.',
  '// It has its own geometry format, so keep one reader and writer to move between that format and GeoJSON.',
  '// Drop repeated points and rings that have collapsed before giving the shape to JSTS.',
  'const samePosition = (first: Position, second: Position) =>',
  '  first[0] === second[0] && first[1] === second[1]',
  'const tidyRing = (ring: Position[]) => {',
  '  const points = ring.filter((point, index) => index === 0 || !samePosition(point, ring[index - 1]!))',
  '  if (points.length > 0 && !samePosition(points[0]!, points.at(-1)!)) points.push(points[0]!)',
  '  return points.length >= 4 ? points : undefined',
  '}',
  '// Keep a polygon’s outer ring and any inner rings that still describe an area.',
  'const tidyPolygon = (coordinates: Position[][]) => {',
  '  const [outerRing, ...innerRings] = coordinates.map(tidyRing)',
  '  return outerRing ? [outerRing, ...innerRings.filter((ring): ring is Position[] => ring !== undefined)] : undefined',
  '}',
  '// Apply that cleanup to either kind of polygonal GeoJSON shape.',
  'const tidyOverlay = (feature: Feature<Polygon | MultiPolygon>) => {',
  "  const polygons = feature.geometry.type === 'Polygon'",
  '    ? [tidyPolygon(feature.geometry.coordinates)]',
  '    : feature.geometry.coordinates.map(tidyPolygon)',
  '  const completePolygons = polygons.filter((polygon): polygon is Position[][] => polygon !== undefined)',
  "  if (completePolygons.length === 0) throw new Error('Overlay has no area after removing collapsed rings.')",
  '  return {',
  '    ...feature,',
  "    geometry: feature.geometry.type === 'Polygon'",
  "      ? { type: 'Polygon' as const, coordinates: completePolygons[0]! }",
  "      : { type: 'MultiPolygon' as const, coordinates: completePolygons },",
  '  }',
  '}',
  'const geometryReader = new GeoJSONReader(new GeometryFactory())',
  'const geometryWriter = new GeoJSONWriter()',
  '// Keep six decimal places: in Hong Kong this grid is about 0.1 m wide.',
  'const overlayPrecision = new PrecisionModel(1_000_000)',
  '// Use this helper only after Turf has created a whole District’s excluded or liveable shape.',
  'const repairOverlay = (feature: Feature<Polygon | MultiPolygon>) => {',
  '  // Read the GeoJSON, align nearby points to the grid, then write GeoJSON again.',
  '  const geometry = geometryWriter.write(GeometryPrecisionReducer.reduce(',
  '    geometryReader.read(tidyOverlay(feature).geometry), overlayPrecision,',
  '  ))',
  '  // Do not save an unexpected shape: map fill layers require a Polygon or MultiPolygon.',
  ...
  '  }',
  '  // Keep the District information and replace only its now-safe geometry.',
