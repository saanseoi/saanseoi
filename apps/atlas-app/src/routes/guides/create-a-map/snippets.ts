...
export const urbanDensityLiveableAreaCode = [
  ...
  '',
  "const nonLiveableFeatures = map.querySourceFeatures('basemap', {",
  "  sourceLayer: 'landuse', filter: ['in', 'kind', ...nonLiveableLandUse],",
...
export const urbanDensityMetricsCode = [
  ...
  `    <span>people per km² on \${metric.landAreaSqKm.toFixed(1)} km²</span>`,
  '  </article>',
  "`).join('')",
  'document.body.append(metricBar)',
].join('\n')
