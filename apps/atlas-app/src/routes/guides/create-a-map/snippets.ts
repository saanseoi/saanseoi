...
export const createUrbanDensityStatsCode = (
  ...
  [
    ...
    'let metrics: AreaMetric[] = savedResult?.metrics ?? []',
    '',
    'if (!savedResult) {',
...
export const urbanDensitySetupZ14TileFetcherCode = [
  ...
  '}',
  '',
  '// Vector-tile fragments are snapped independently, so their finished edges may not line up.',
  '// GEOS is the geometry engine behind PostGIS and Shapely; geos-wasm runs that tested engine in the browser.',
  '// Load its larger WebAssembly file only when a District has a finished shape to repair.',
  'const geosReady = Promise.all([',
  "  import('geos-wasm'),",
  ...
  '  ...helpers,',
  '}))',
  '// Making a shape valid can also return lines, so keep only its original polygon pieces.',
  ...
  '      : []',
  '// Preserve a valid overlay exactly; only send invalid output through MakeValid.',
  'const repairOverlay = async (feature: Feature<Polygon | MultiPolygon>) => {',
  '  const { geos, geojsonToGeosGeom, geosGeomToGeojson } = await geosReady',
  '  let input = 0',
  '  let madeValid = 0',
  '  let polygonal = 0',
  '  try {',
  '    input = geojsonToGeosGeom(feature.geometry, geos)',
  '    if (geos.GEOSisValid(input) === 1) return feature',
  '    // Resolve crossing edges and collapsed rings only when the overlay is invalid.',
  '    madeValid = geos.GEOSMakeValid(input)',
  '    const coordinates = polygonalCoordinates(geosGeomToGeojson(madeValid, geos) as Geometry)',
  "    if (coordinates.length === 0) throw new Error('Overlay repair left no polygonal land.')",
  '    // Keep disconnected land pieces separate: a precision union could bridge a real gap.',
  "    const geometry = { type: 'MultiPolygon' as const, coordinates }",
  '    polygonal = geojsonToGeosGeom(geometry, geos)',
  '    // Refuse invalid output: MapLibre fill layers need one valid polygonal geometry.',
  '    if (geos.GEOSisValid(polygonal) !== 1) {',
  "      throw new Error('Overlay repair did not produce valid polygonal geometry.')",
  '    }',
  '    return { ...feature, geometry }',
  '  } finally {',
  '    // Release each temporary GEOS shape before continuing with the next District.',
  '    if (polygonal) geos.GEOSGeom_destroy(polygonal)',
  '    if (madeValid) geos.GEOSGeom_destroy(madeValid)',
  '    if (input) geos.GEOSGeom_destroy(input)',
  '  }',
  '}',
  '// Exclusions take precedence: no saved liveable feature may overlap one.',
  'const removeExcludedOverlap = async (liveable: Feature<Polygon | MultiPolygon>, excluded: Feature<Polygon | MultiPolygon>) => {',
  '  const overlap = intersect(featureCollection([liveable, excluded]))',
  '  if (!overlap || area(overlap) === 0) return liveable',
  '  const disjoint = difference(featureCollection([liveable, excluded]))',
  "  if (!disjoint) throw new Error('Excluded land covers this entire District.')",
  '  return repairOverlay(disjoint)',
  '}',
  ...
  "    .replace('{x}', String(x))",
  "    .replace('{y}', String(y))",
  'const decodedTiles = new Map<string, Feature<Polygon>[]>()',
  ...
  '      return isArea ? polygonParts(geojson as Feature<Polygon | MultiPolygon>) : []',
...
export const urbanDensityCollectNonLiveableLandCode = [
  'const nonLiveableFeatures: Feature<Polygon>[] = []',
  'let exclusions: Array<{ feature: Feature<Polygon>; bounds: Bounds }> = []',
  'if (!savedResult) {',
  '  try {',
  ...
  '    clearTileOutline()',
  '  }',
  '  exclusions = nonLiveableFeatures.map(feature => ({ feature, bounds: boundsFor(feature) }))',
  '}',
].join('\n')
...
export const urbanDensityLiveableAreaCode = [
  ...
  'const districtLand: Array<{ district: District; excluded: Feature<Polygon | MultiPolygon>; liveable: Feature<Polygon | MultiPolygon> }> = []',
  'const completedExclusions: Array<Feature<Polygon | MultiPolygon, { area: string }>> = []',
  ...
  '  await pauseForAir()',
  '  const districtPolygonParts = polygonParts(district).map(feature => ({ feature, bounds: boundsFor(feature) }))',
  '  showDistrictProgress(index, district, 0, districtPolygonParts.length)',
  '  const clippedExclusions: Feature<Polygon | MultiPolygon>[] = []',
  '  let clippedCount = 0',
  ...
  '      }',
  '    }',
  '    showDistrictProgress(index, district, partIndex + 1, districtPolygonParts.length)',
  '    await pauseForAir()',
  '  }',
  '  // Turf builds the coverage; GEOS repairs only an invalid completed overlay.',
  '  const mergedExclusions = union(featureCollection(clippedExclusions))',
  "  if (!mergedExclusions) throw new Error('No excluded land overlaps this District.')",
  '  const excluded = await repairOverlay(mergedExclusions)',
  '  const remainingLand = difference(featureCollection([district, excluded]))',
  "  if (!remainingLand) throw new Error('Excluded land covers this entire District.')",
  '  const liveable = await removeExcludedOverlap(await repairOverlay(remainingLand), excluded)',
  ...
  '  showDistrictProgress(index + 1, district, districtPolygonParts.length, districtPolygonParts.length)',
  '}',
  '',
  'const analysisResult = {',
  '  districts,',
  '  metrics,',
  "  liveableDistrictLand: { type: 'FeatureCollection' as const, features: districtLand.map(({ district, liveable }) => ({ ...liveable, properties: district.properties })) },",
  "  excludedDistrictLand: { type: 'FeatureCollection' as const, features: districtLand.map(({ district, excluded }) => ({ ...excluded, properties: district.properties })) },",
  '}',
...
export const urbanDensityLiveableMetricsCode = [
  ...
  'const liveableTotalsByArea = excludedDistrictLand.reduce((totals, district) => {',
