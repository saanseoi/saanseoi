...
export const urbanDensitySetupZ14TileFetcherCode = [
  ...
  "progressPanel.style.cssText = 'position: fixed; left: 50%; bottom: 1rem; z-index: 1; width: min(calc(100% - 2rem), 32rem); transform: translateX(-50%); border: 1px solid rgba(255, 255, 255, 0.2); background: rgba(16, 21, 26, 0.95); padding: 1rem; color: white; text-align: center; font-family: system-ui, sans-serif'",
  ...
  "progressDistrict.style.cssText = 'margin: 0; color: #79e7d1; font-family: ui-monospace, monospace; font-size: 1.5rem; font-weight: 700; letter-spacing: -0.025em'",
  "const progressCount = document.createElement('p')",
  "progressCount.style.cssText = 'margin: 0.25rem 0 0; color: rgba(255, 255, 255, 0.85); font-family: ui-monospace, monospace; font-size: 1rem; font-weight: 600; font-variant-numeric: tabular-nums'",
  "const progress = document.createElement('progress')",
  "progress.style.cssText = 'display: block; width: 100%; height: 0.5rem; margin-top: 0.75rem; accent-color: #43c6ad'",
  'progressPanel.append(progressPhase, progressDistrict, progressCount, progress)',
  ...
  '    map.fitBounds([west, south, east, north], { padding: 48, duration: 500, maxZoom: 12 })',
  '  }',
  '  const countLabel = `${completedParts.toLocaleString()} / ${totalParts.toLocaleString()} parts for ${completed.toLocaleString()} / ${districts.length.toLocaleString()} districts`',
  "  setProgress(progressNode, completed, districts.length, district, '[CALCULATE DISTRICT INTERSECTION]', countLabel)",
  '}',
].join('\n')
