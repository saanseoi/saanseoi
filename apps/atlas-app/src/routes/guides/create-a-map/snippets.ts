...
export const urbanDensitySetupZ14TileFetcherCode = [
  ...
  "import { area, bbox, bboxPolygon, featureCollection } from '@turf/turf'",
  ...
  '  geometryWorker = undefined',
  '}',
  '// GEOS runs in a worker, so topology never blocks map interaction or progress rendering.',
  ...
  '}',
  "const showTileProgress = (completed: number, total: number, tile: ProcessingTile, phase = '[DOWNLOAD, SNAP & MERGE TILE]') => {",
  '  setProgressCounts(completed, total, 0, undefined, districts.indexOf(tile.district))',
  '  setProgress(progressNode, completed, total, tile.district, phase)',
  '}',
  ...
  '  setProgressCounts(totalParts, totalParts, completedParts, totalParts, completed)',
  '  setProgress(progressNode, completedParts, totalParts, district, phase)',
  '}',
].join('\n')
...
export const urbanDensitySetupZ14TileFetcherCss = [
  ...
  '  width: min(calc(100% - 2rem), 32rem); transform: translateX(-50%);',
  ...
  '}',
  '#land-analysis-progress .land-analysis-progress-counts {',
  '  display: grid; grid-template-columns: 1fr auto 1fr auto 1fr; align-items: end; gap: 0.75rem;',
  '  margin: 0.75rem 0 0;',
  '}',
  ...
  '  #land-analysis-progress .land-analysis-progress-counts { margin-top: .625rem; gap: .4rem; }',
...
export const urbanDensityCollectNonLiveableLandCode = [
  ...
  '      showTileProgress(index, tiles.length, tile)',
  ...
  '      showTileProgress(index + 1, tiles.length, tile)',
