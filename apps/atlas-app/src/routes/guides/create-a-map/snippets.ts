...
export const urbanDensityLiveableAreaCode = [
  ...
  '  const excluded = await repairOverlay(union(featureCollection(clippedExclusions)))',
  '  // Make the liveable result valid too, preserving every separate land piece.',
  '  const liveable = await repairOverlay(difference(featureCollection([district, excluded])))',
