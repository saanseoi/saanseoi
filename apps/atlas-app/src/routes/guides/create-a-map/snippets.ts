...
export const urbanDensitySetupZ14TileFetcherCode = [
  ...
  '}',
  '',
  '// GEOS is the geometry engine behind PostGIS and Shapely; geos-wasm runs it in the browser.',
  ...
  '  }',
  '}',
  'const differenceAnalysisGeometries = async (first: Polygon | MultiPolygon, second: Polygon | MultiPolygon) => {',
  '  const { geos, geojsonToGeosGeom, geosGeomToGeojson } = await geosReady',
  '  let firstGeometry = 0',
  '  let secondGeometry = 0',
  '  let remainder = 0',
  ...
  '    firstGeometry = geojsonToGeosGeom(first, geos)',
  '    secondGeometry = geojsonToGeosGeom(second, geos)',
  '    remainder = geos.GEOSDifferencePrec(firstGeometry, secondGeometry, precisionGrid)',
  "    if (!remainder || geos.GEOSisEmpty(remainder) === 1) throw new Error('Excluded land covers this entire District.')",
  '    return polygonalGeometry(',
  '      geosGeomToGeojson(remainder, geos) as Geometry,',
  "      'Liveable land difference is not polygonal.',",
  '    )',
  '  } finally {',
  '    if (remainder) geos.GEOSGeom_destroy(remainder)',
  '    if (secondGeometry) geos.GEOSGeom_destroy(secondGeometry)',
  '    if (firstGeometry) geos.GEOSGeom_destroy(firstGeometry)',
  '  }',
  '}',
  'const mergeTileCoverage = async (tile: { x: number; y: number }, features: Feature<Polygon>[]) => {',
  ...
  "    .replace('{x}', String(x))",
  "    .replace('{y}', String(y))",
  'const decodedTiles = new Map<string, Polygon | MultiPolygon | undefined>()',
  ...
  '    : []',
  '',
  '  // Union overlapping categories, then remove the tile buffer so this tile owns one exact core.',
  '  const coverage = await mergeTileCoverage(tile, features)',
  '  decodedTiles.set(key, coverage)',
  '  return coverage',
  '}',
  '',
...
export const urbanDensityCollectNonLiveableLandCode = [
  ...
  '      const geometry = await fetchNonLiveableLand(tile)',
...
export const urbanDensityLiveableAreaCode = [
  ...
  "  showDistrictProgress(index, district, candidateTiles.length, candidateTiles.length, '[BUILD LIVEABLE LAND]')",
  '  await pauseForAir()',
  '  const liveableGeometry = await differenceAnalysisGeometries(districtGeometry, excludedGeometry)',
  '  const excluded = { ...district, geometry: fromAnalysisGeometry(excludedGeometry) }',
  '  const liveable = { ...district, geometry: fromAnalysisGeometry(liveableGeometry) }',
