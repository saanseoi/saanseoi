...
export const urbanDensitySetupZ14TileFetcherCode = [
  ...
  "import { area, bbox, bboxPolygon, booleanIntersects, booleanValid, cleanCoords, featureCollection, flatten, intersect, unkinkPolygon } from '@turf/turf'",
  ...
  'const repairGeometry = async (source: Polygon | MultiPolygon) => {',
  ...
  '    input = geojsonToGeosGeom(source, geos)',
  ...
  '  }',
  '}',
  '// Use GEOS for the finished District overlay, without snapping or joining across a gap.',
  'const mergeExclusions = async (features: Array<Feature<Polygon | MultiPolygon>>) => {',
  '  const { geos, geojsonToGeosGeom, geosGeomToGeojson } = await geosReady',
  '  let collection = 0',
  '  let merged = 0',
  '  try {',
  "    collection = geojsonToGeosGeom({ type: 'GeometryCollection', geometries: features.map(feature => feature.geometry) }, geos)",
  '    // This exact union removes overlaps, but never moves nearby edges onto a grid.',
  '    merged = geos.GEOSUnaryUnion(collection)',
  '    const geometry = geosGeomToGeojson(merged, geos) as Geometry',
  "    if (geometry.type !== 'Polygon' && geometry.type !== 'MultiPolygon') throw new Error('Excluded land union is not polygonal.')",
  ...
  '  }',
  '}',
  '// Subtract with the same engine, so both completed boundaries have one topology model.',
  'const subtractExcludedLand = async (district: Polygon | MultiPolygon, excluded: Polygon | MultiPolygon) => {',
  '  const { geos, geojsonToGeosGeom, geosGeomToGeojson } = await geosReady',
  '  let districtGeometry = 0',
  '  let excludedGeometry = 0',
  '  let remaining = 0',
  '  try {',
  '    districtGeometry = geojsonToGeosGeom(district, geos)',
  '    excludedGeometry = geojsonToGeosGeom(excluded, geos)',
  '    remaining = geos.GEOSDifference(districtGeometry, excludedGeometry)',
  '    const geometry = geosGeomToGeojson(remaining, geos) as Geometry',
  "    if (geometry.type !== 'Polygon' && geometry.type !== 'MultiPolygon') throw new Error('Liveable land difference is not polygonal.')",
  '    return repairGeometry(geometry)',
  '  } finally {',
  '    if (remaining) geos.GEOSGeom_destroy(remaining)',
  '    if (excludedGeometry) geos.GEOSGeom_destroy(excludedGeometry)',
  '    if (districtGeometry) geos.GEOSGeom_destroy(districtGeometry)',
  '  }',
  '}',
...
export const urbanDensityLiveableAreaCode = [
  ...
  '  const excluded = { ...district, geometry: await mergeExclusions(clippedExclusions) }',
  '  // Make the liveable result valid too, preserving every separate land piece.',
  '  const liveable = { ...district, geometry: await subtractExcludedLand(district.geometry, excluded.geometry) }',
