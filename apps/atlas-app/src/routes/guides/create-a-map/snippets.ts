...
export const urbanDensityCalculationCode = [
  ...
  '',
  '// Link each district code to its Area so we can combine district statistics into Hong Kong Island, Kowloon and the New Territories.',
  ...
  ')',
  '',
  '// Go through the population values returned by the Statistics API and add each district to its Area total.',
  ...
  '',
  '    const total = totals.get(area.id) ?? { name: area.name, population: 0, landAreaSqKm: 0 }',
...
export const urbanDensityMapCode = [
  ...
  '',
  '// Quieten open and non-residential land-use areas.',
  ...
  '',
  '// Add a subdued context layer without changing the density calculation.',
  'map.addLayer({',
  "  id: 'green-context',",
  "  type: 'fill',",
  "  source: 'basemap',",
