...
function printUsage() {
  ...
  bun run dataops -- hkgov-dpo:ingest <ALS-source-root> --target local|preview|production --cohort-key START_COHORT [--from-source-version YYYY-MM-DD.NNNN] [--identity-history FILE] [--identity-decisions FILE] [--release-notes-url URL] [--defer-api-release-set] [--dry-run] [--yes]
  ...
  bun run dataops -- hkgov-censtatd:statistics <source.zip> --target local|preview|production --dataset-code CODE --source-version PERIOD --release-notes-url URL --source-archive-key KEY --source-archive-sha256 SHA256 [--geography-only|--include-geography] [--defer-api-release-set] [--force-upload]
...
async function main() {
  ...
  switch (args.command) {
    ...
    case 'hkgov-censtatd:statistics': {
      const { runHkgovCenstatdStatisticsIngestCommand } = await import(
        './commands/hkgovCenstatdStatistics.ts'
      )
      await runHkgovCenstatdStatisticsIngestCommand(args, target, printUsage)
      return
    }
    case 'hkgov-had:district-area': {
      const { runHkgovHadDistrictArchiveIngestCommand } = await import(
