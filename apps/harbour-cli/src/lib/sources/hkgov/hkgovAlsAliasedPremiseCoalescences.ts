...
export function coalesceAlsAliasedPremises(
  ...
  for (const decision of fixture.coalescences) {
    ...
    for (const row of [owner, alias]) {
      ...
      assert.equal(row.enBlockDescriptor, 'BLK')
      assert.equal(row.enBlockNumber, decision.blockRef)
      assert.equal(row.zhHantBlockDescriptor, '座')
      assert.equal(row.zhHantBlockNumber, decision.blockRef)
