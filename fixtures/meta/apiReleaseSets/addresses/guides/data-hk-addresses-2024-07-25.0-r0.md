...
domainCode: "saanseoi"
timeTravelEffectiveAt: "2024-07-25T00:00:00.000Z"
timeTravelKnownAt: "2026-09-09T00:26:38.567Z"
timeTravelCatalogRevision: "catalog-hk-addresses-v0.1-2026-09-09.0"
---
...
[Addressess source-record endpoint](/docs#tag/Sources/operation/listAddressSourceRecordsV0)
...
完整參考請見[Addresses API 文件](/docs#tag/Addresses/operation/listAddressesV01)。

本指南說明如何向 Addresses API 發出請求。API <i>回應</i>的結構及內容，請參閱
...
<black>saanseoi</black> domain 中<black>最新</black> cohort 的記錄。
...
<black>meta.apiCatalogRevision</black> 記錄解析出的發布。

## 搜尋地址

搜尋端點必須提供查詢及配對模式。搜尋接受與 collection 相同的 release、domain、cohort、時間回溯、profile、locale、hierarchy 及 Division
filter，因此可重現已封存的發布視圖。
...
- <black>full-text</black>
  在格式化地址及組成部分文字中尋找所有查詢 token。常見英文縮寫如 <i>BLK</i>、<i>TWR</i>
  及 <i>HSE</i> 會與完整寫法互相配對。
- <black>prefix</black> 配對每個索引 token 的開首。
- <black>component</black> 只搜尋一個指定組成部分，須提供
  <black>component=formatted|building|number|block|phase|estate|street</black>。
- <black>exact</black> 接受一個樓宇號碼 token，例如 <black>427</black> 或
...
Filter 會在分頁前收窄清單或搜尋結果。使用 <black>filter[dataset]</black>
...
每個 Address resource 均帶有標準 Division 關係識別碼。如亦需要頂層
<black>included</black> array 中的相關 Division resource，請使用
<black>include=hierarchy</black>。

## 取得單位

Address resource 的 <black>attributes.address3dCoverage</black>
說明 Address3D 單位 collection 是直接可用，還是透過已確立或未解析的祖先可用。使用以下請求取得：
...
如有 collection，回應會包含樓層及單位參照和本地化格式化部分。<black>data</black>
...
使用 <black>profile=full</black> 取得 Address snapshot ID、政府識別碼及
...
- detail 或 units 請求傳回
  `404`，表示所選發布沒有該 ID。請重新檢查 ID 及其 release、domain、cohort 和時間回溯 selector。
- `422`
  表示請求無效。請閱讀驗證詳情，再更正 selector、filter、locale、配對模式、component、搜尋或分頁值。
- `503` 及 <black>snapshot_not_ready</black> 表示沒有符合選擇的 Address
  snapshot。請在發布後重試或選擇已發布的 release；不要視作空白結果。
...
完整参考请见[Addresses API 文档](/docs#tag/Addresses/operation/listAddressesV01)。

本指南说明如何向 Addresses API 发出请求。API <i>响应</i>的结构及内容，请参阅
...
<black>saanseoi</black> domain 中<black>最新</black> cohort 的记录。

如要获取此特定发布，请同时提供两个 selector：
...
<black>meta.apiCatalogRevision</black> 记录解析出的发布。

## 搜索地址

搜索端点必须提供查询及匹配模式。搜索接受与 collection 相同的 release、domain、cohort、时间回溯、profile、locale、hierarchy 及 Division
filter，因此可重现已归档的发布视图。
...
- <black>full-text</black>
  在格式化地址及组成部分文本中查找所有查询 token。常见英文缩写如 <i>BLK</i>、<i>TWR</i>
  及 <i>HSE</i> 会与完整写法相互匹配。
- <black>prefix</black> 匹配每个索引 token 的开头。
- <black>component</black> 只搜索一个指定组成部分，须提供
  <black>component=formatted|building|number|block|phase|estate|street</black>。
- <black>exact</black> 接受一个楼宇号码 token，例如 <black>427</black> 或
...
Filter 会在分页前缩小列表或搜索结果。使用 <black>filter[dataset]</black>
...
每个 Address resource 均带有标准 Division 关系标识符。如也需要顶层
<black>included</black> array 中的相关 Division resource，请使用
<black>include=hierarchy</black>。

## 获取单位

Address resource 的 <black>attributes.address3dCoverage</black>
说明 Address3D 单位 collection 是直接可用，还是通过已确立或未解析的祖先可用。使用以下请求获取：
...
如有 collection，响应会包含楼层及单位引用和本地化格式化部分。<black>data</black>
...
使用 <black>profile=full</black> 获取 Address snapshot ID、政府标识符及
...
- detail 或 units 请求返回
  `404`，表示所选发布没有该 ID。请重新检查 ID 及其 release、domain、cohort 和时间回溯 selector。
- `422`
  表示请求无效。请阅读验证详情，再更正 selector、filter、locale、匹配模式、component、搜索或分页值。
- `503` 及 <black>snapshot_not_ready</black> 表示没有符合选择的 Address
  snapshot。请在发布后重试或选择已发布的 release；不要视作空白结果。
- `503` 及 <black>fts_not_ready</black>
