...
domain 獨立進行版本控制，並非預設 geographic 集合的篩選結果。它包含 規劃署新市鎮區劃 snapshot，以及與其完全對應的面配套資源；結果絕不混入其他 domain 的記錄。
...
此 <black>{{ domainCode }}</black>
domain 独立进行版本控制，并非默认 geographic 集合的筛选结果。它包含 规划署新市镇区划 snapshot，以及与其完全对应的面配套资源；结果绝不混入其他 domain 的记录。
