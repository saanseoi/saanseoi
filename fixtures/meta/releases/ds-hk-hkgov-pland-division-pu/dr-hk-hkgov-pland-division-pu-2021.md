...
dataset: "ds-hk-hkgov-pland-division-pu"
release: "dr-hk-hkgov-pland-division-pu-2021"
regionCode: "hk"
...
type: "division"
cohortKey: "2021"
releaseNotesUrl: "https://portal.csdi.gov.hk/geoportal/?lang=en&datasetId=pland_rcd_1634022783366_65050"
---
...
- Planning Department Planning Unit and subunit cohort 2021.

## Geometry

Canonical division geometry is derived from accepted Polygon and MultiPolygon source
cells. Reviewed invalid rings use a `buffer(0)` repair only for canonical geometry;
original publisher geometry remains retained.
...
- `PPU`, `SPU`, `TPU`, and `Subunit` - retained in the Planning Department source
  profile
...
標準區劃幾何由已接受的 Polygon 和 MultiPolygon 來源格網衍生。經審核的無效環僅會以
...
- `PPU`、`SPU`、`TPU` 和 `Subunit` 保留於規劃署來源 profile
...
- `PPU`、`SPU`、`TPU` 和 `Subunit` 可透過 <black>hkgovPland</black> 來源 profile 取得
- 修復狀態及來源格網證據隨來源主張保留
...
标准区划几何由已接受的 Polygon 和 MultiPolygon 源格网衍生。经审核的无效环仅会以
`buffer(0)` 修复标准几何；原始发布者几何仍会保留。
...
- `PPU`、`SPU`、`TPU` 和 `Subunit` 保留于规划署源 profile
...
- `PPU`、`SPU`、`TPU` 和 `Subunit` 可通过 <black>hkgovPland</black> 源 profile 取得
- 修复状态及源格网证据随源主张保留
