...
dataset: "ds-hk-hkgov-censtatd-division-statistic-permanent-living-quarters-district"
release: "dr-hk-hkgov-censtatd-division-statistic-permanent-living-quarters-district-2023-H2"
regionCode: "hk"
source: "hkgov-censtatd"
sourceVersion: "2023-H2"
releaseNotesUrl: "https://portal.csdi.gov.hk/geoportal/?lang=en&datasetId=censtatd_rcd_1635934103275_66203"
releaseVersion: "2023-H2.0"
type: "divisionStatistic"
cohortKey: "2023-H2"
hkgovCenstatdCuration: "fixtures/meta/curations/hkgov-censtatd-statistics/permanent-living-quarters-district.json"
measureCuration: "fixtures/meta/curations/hkgov-censtatd-statistics-measures/permanent-living-quarters-district.json"
---
...
- <orange>Upstream</orange> Add [C&SD](saanseoi:en:definition/hkgov-censtatd/v1)'s
  {{ sourceVersion }} Permanent Living Quarters by District Council District statistics.

## Versioning

The version <black>{{ releaseVersion }}</black> preserves C&SD's reviewed
<black>{{ sourceVersion }}</black> publisher release. Its source period and District
Council statistical geography come from the native layer, not a CSDI archive slot.
...
- <orange>上游</orange> 新增 [C&SD](saanseoi:zh-hant:definition/hkgov-censtatd/v1) 的
  {{ sourceVersion }} 年下半年按區議會分區劃分的永久性居住屋宇統計資料。

## 版本控制

版本 <black>{{ releaseVersion }}</black> 保留 C&SD 經審核的
<black>{{ sourceVersion }}</black>
發布。其來源時段及區議會統計地理範圍取自原生圖層，而非 CSDI 歸檔時段。
...
- <orange>上游</orange> 新增 [C&SD](saanseoi:zh-hans:definition/hkgov-censtatd/v1) 的
  {{ sourceVersion }} 年下半年按区议会分区划分的永久性居住屋宇统计数据。

## 版本控制

版本 <black>{{ releaseVersion }}</black> 保留 C&SD 经审核的
<black>{{ sourceVersion }}</black>
发布。其来源时段及区议会统计地理范围取自原生图层，而非 CSDI 归档时段。
