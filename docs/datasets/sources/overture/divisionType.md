...
1. Normalize `subtype` and `class`, then read a string value from `admin_level` or
   `adminLevel` when available.
2. Check the normalized `subtype`, then `class`, then the string admin-level value for a
   recognized level token. The first matching token supplies the level.
3. If no recognized token is found, use level `1` when the row has a parent division;
   otherwise use level `0`.
4. If no direct type mapping is available, derive the generic type from the resulting
   level: `0` → `SAR`, `1` → `area`, `2` → `district`, `3` → `town`, `4` → `macrohood`,
   `5` → `neighbourhood`, and `6` → `microhood`.
...
1. 正規化 `subtype` 和 `class`，然後在可用時讀取 `admin_level` 或 `adminLevel`
   的字串值。
2. 依次檢查正規化後的 `subtype`、`class`
   及字串形式的行政層級值，尋找已識別的層級 token。第一個符合的 token 會決定層級。
3. 如果找不到已識別的 token，資料列有父區劃時使用層級 `1`，否則使用層級 `0`。
4. 如果沒有直接的類型對應，便根據所得層級推導通用類型：`0` → `SAR`、`1` → `area`、 `2` →
   `district`、`3` → `town`、`4` → `macrohood`、`5` → `neighbourhood`，以及 `6` →
   `microhood`。

`locality` 對應需要兩個欄位。例如，`subtype: locality` 配合 `class: village`
會直接對應至層級 `5` 及類型 `village`。如果只有
`class: village`，仍可從已識別的類別 token 恢復層級，但會使用層級 `5`
的通用類型。數值形式的 `admin_level`
...
1. 规范化 `subtype` 和 `class`，然后在可用时读取 `admin_level` 或 `adminLevel`
   的字符串值。
2. 依次检查规范化后的 `subtype`、`class`
   以及字符串形式的行政层级值，寻找已识别的层级 token。第一个匹配的 token 会决定层级。
3. 如果找不到已识别的 token，数据行有父区划时使用层级 `1`，否则使用层级 `0`。
4. 如果没有直接的类型映射，则根据所得层级推导通用类型：`0` → `SAR`、`1` → `area`、 `2` →
   `district`、`3` → `town`、`4` → `macrohood`、`5` → `neighbourhood`，以及 `6` →
   `microhood`。

`locality` 映射需要两个字段。例如，`subtype: locality` 配合 `class: village`
会直接映射到层级 `5` 及类型 `village`。如果只有
`class: village`，仍可从已识别的类别 token 恢复层级，但会使用层级 `5`
的通用类型。数值形式的 `admin_level`
