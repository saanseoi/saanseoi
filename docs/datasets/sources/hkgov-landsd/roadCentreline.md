...
Source English and Traditional Chinese names are retained as source i18n. The
publisher's `LASTUPDATEDATE` and `SHAPE_Length` fields are dropped.
...
canonical state live in separate D1 shards. Unmatched or ambiguous records must be
represented in the versioned curation fixture before publication.
