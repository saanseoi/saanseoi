...
| Existing web embed | WordPress, Squarespace, Wix, Webflow or generic iframe after a standalone deployment        | Local-only URL preview, responsive iframe generator and placement help  | Direct `pk.` key and regional TileJSON | SaanSeoi carousel and custom-style prompt | Own-data prompt and urban-density calculation example | Needed      |
